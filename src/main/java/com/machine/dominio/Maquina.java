...
public class Maquina {
	...
	/**
	 * Este metodo a partir de un refresco en especifico
	 * da la cantidad de ellos disponible.
	 * @param refresco
	 * @return Devuelve un int con la cantidad de refrescos 
	 * especificos que se estan buscando.
	 * @author Jon Mateo Bengoetxea
	 */
	...
	/**
	 * Visualiza un informe completo de todos los productos comprados. 
	 * (Productos y cantidad de dinero ganado)
	 * @author Jon Mateo Bengoetxea
	 */
	...
	/**
	 * Este metodo realiza la accion de comprar un refresco.
	 * Verifica el dinero introducido es correcto para el precio del producto y si se encuentra disponible
	 * realiza la compra. Remueve de la maquina la bebida comprada y la anota para posteriormente sacar
	 * un informe de todas las compras realizadas.
	 * @param Recibe un objeto refresco
	 * @param Recibe el dinero que ha depositado el cliente en la maquina
	 * @author Jon Mateo Bengoetxea
	 */
	public void comprarRefresco(Refresco refresco, Double dinero) {
		...
		if (dinero >= precio) {
			if (buscarRefresco(refresco)==true) {
				if (this.dinero-dinero >= 0) {
					...
					System.err.println("Compra realizada, vueltas: " + (dinero-precio));
				}else {
					System.err.println("No quedan vueltas en la maquina.");
				}
			}else {
				System.err.println("No queda este refresco en la maquina.");
			}
		}else {
			System.err.println("No tienes suficiente dinero!");
		}
		
	}
	
	/**
	 * Muestra el estado actual de la maquina al completo. (Dinero y Stock disponible)
	 * @author Jon Mateo Bengoetxea
	 */
	public void mostrarEstadoActual() {
		System.out.println("--------- ESTADO ACTUAL ---------");
		System.out.println("Dinero actual: "+this.dinero+" || Stock: "+this.stock + " | " +getRefrescos().size() + " refrescos.");
		 for (int i=0;i<this.refrescos.size();i++) {
	 ...
	 * @return Devuelve un booleano, false para indicar que no esta y true
	 * para indicar que si esta.
	 */
	 ...
	 * @author Jon Mateo Bengoetxea 
	 * @param Se le entrega un objeto Refresco.
	 * @return Devuelve la posicion (int) en la que se encuentra el refresco en el array,
	 * si el valor es negativo no se encuentra y si es positivo te da su posicion.
	 */
	...
	/**
	 * Este metodo recorre los refrescos de la maquina y calcula
	 * el precio de los mismos para de esta manera saber cuanto
	 * dinero de ganancias se ha acomulado en la maquina. 
	 * @author Jon Mateo Bengoetxea 
	 * @return Devuelve un double con la cantidad total (en dinero) de refrescos vendidos.
	 */
	...
	/**
	 * Este metodo muestra el stock y la disponibilidad
	 * de un refresco en especifico en la maquina.
	 * @author Jon Mateo Bengoetxea
